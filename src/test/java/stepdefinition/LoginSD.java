...
public class LoginSD {
    ...
    @When("^I click on (password|email) text field on home screen$")
    public void clickOnPasswordTextField(String textField){
        switch (textField){
            case "password":
                homePage.clickOnPasswordTextField();
                break;
            case "email":
                homePage.clickOnEmailTextField();
        }
    }
    ...
    public void verifyInvalidEmailErrorMassage(){
        Assert.assertEquals(homePage.getErrorMessage(), "Please enter a valid mobile number or email address.");
    }
}
